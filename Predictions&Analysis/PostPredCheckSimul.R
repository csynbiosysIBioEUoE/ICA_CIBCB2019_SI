...
# same, predictions for all experimental results using all the posteriors obtained will be performed. 
...
PPCSimul <- function(experiment, experiment2){
  
  expose_stan_functions("OED_Model_Function.stan")
  ...
  for(i in 1:length(experiment2)){
    ...
    for(j in 1:length(experiment)){
      ...
      resnam1 <- paste("InferenceResults_", experiment2[i], "_Simulation_", experiment[j], "_RFP.csv", sep = "")
      resnam2 <- paste("InferenceResults_", experiment2[i], "_Simulation_", experiment[j], "_GFP.csv", sep = "")
