...
generated quantities {
  ...
  // Section to compute and extract the loglikelihood values for each parameter sample drawn at each iteration
  ...
  for (j in 1:m){
    ...
    for (q in 1:Nsp[1,j]-1){
      ...
      int itp = evnT[q,j]; 
      int lts = num_elements(ts[(evnT[q,j]+1):(evnT[q+1,j]+1),j]);  
      real part1[lts,Neq];
